...
class Route
{
    ...
    public function __construct(Container $container, array $handler, array $params)
    {
        ...
        if(!is_null($handler['class']) && !is_null($handler['method'])) {
            $this->resolver = function(Request $request) use ($handler, $params) {
                return $this->resolveClassMethod($request, $params, $handler['class'], $handler['method']);
            };
        } else if(!is_null($handler['closure'])) {
            $this->resolver = function(Request $request) use ($handler, $params) {
                return $this->resolveClosure($request, $params, $handler['closure']);
            };
        } else {
